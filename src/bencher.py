...
def run_bench(args, settings):
    ...
    param_cfg['bench']['working_path'] = gs.base_dir + \
        gs.sl + 'build' + gs.sl + code_path + gs.sl + session
    # Path to application's data directory
    param_cfg['bench']['dataset_path'] = gs.base_dir + \
        gs.sl + 'datasets' + gs.sl + code
    # Directory to add to MODULEPATH
    param_cfg['bench']['base_mod'] = gs.base_dir + \
        gs.sl + 'build' + gs.sl + 'modulefiles'
    # Directory to application installation
    param_cfg['bench']['app_mod'] = code_path
    # Get total ranks from nodes * ranks_per_node
    param_cfg['sched']['ranks'] = int(
        param_cfg['sched']['nodes']) * int(param_cfg['sched']['ranks_per_node'])
    ...
    builder.submit_job(
        param_cfg['bench']['working_path'] + gs.sl + script_file[4:], logger)
