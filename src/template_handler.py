...
def populate_template(input_cfgs, script_file):
    ...
    for cfg in input_cfgs:
        for key in cfg:
            logger.debug("replace " + "<<<" + str(key) + ">>> with " + str(cfg[key]))
...
def test_template(script):
    ...
    if len(nomatch) > 0:
        ...
        if gs.exit_on_missing:
            exception.error_and_quit(logger, "Missing parameters were found after populating the template and exit_on_missing=True in settings.cfg:"+' '.join(nomatch))
    else:
