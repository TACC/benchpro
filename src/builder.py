...
def check_for_previous_install(path):
    if os.path.exists(path):
        if gs.overwrite:
            logger.debug("WARNING: It seems this app is already installed. Deleting old build in "+path+" because 'overwrite=True' in settings.cfg")
            ...
            exception.error_and_quit(logger, "It seems this app is already installed in "+path+". The install directory already exists and 'overwrite=False' in settings.cfg")
...
def create_install_dir(path, logger):
    ...
    if not os.path.exists(path):
        try:
            ...
            exception.error_and_quit(logger, "Failed to make directory "+path)
...
def install(path, obj, logger):
    ...
    # Get file name
    new_obj_name = obj
    if gs.sl in obj: new_obj_name = obj.split(gs.sl)[-1]
    ...
    try: 
        ...
        print(e)
        exception.error_and_quit(logger, "Failed to move "+obj+" to "+path + gs.sl + new_obj_name)
...
def build_code(args):
    ...
    # Init loggers
    global logger 
    logger = start_logging("BUILD", file=gs.base_dir + gs.sl + gs.build_log_file + "_" + gs.time_str + ".log")
    ...
    build_template      = gs.build_tmpl_dir + gs.sl + code_cfg['general']['code'] + "-" + code_cfg['general']['version'] + ".build"
    compiler_template   = "compile_flags.template" 
    module_template     = gs.build_tmpl_dir + gs.sl + code_cfg['general']['code'] + "-" + code_cfg['general']['version'] + ".module"

    # Use generic module template if not found for this application
    if not os.path.exists(module_template):
        logger.debug("WARNING: "+code_cfg['general']['code']+" module template file not available at "+module_template)
    ...
    mod_path, mod_file = module_handler.make_mod(module_template, code_cfg['general'], code_cfg['build'], code_cfg['modules'], logger)
    ...
    # Submit build script to scheduler
    submit_job(code_cfg['general']['build_prefix'] + gs.sl + script_file[4:], logger)
