...
def check_inputs(mod_dict, mod_path, logger):
    ...
    if os.path.isdir(mod_path):
        ...
        if gs.overwrite:
            logger.debug("WARNING: Deleting old module in "+mod_path+" because 'overwrite=True' in settings.cfg")
            su.rmtree(mod_path)
...
def copy_mod_template(template_file, mod_file, logger):
    try:
        ...
        exception.error_and_quit(logger, "Failed to "+template_file + " to " + mod_file)
...
def populate_mod_template(module, mod_dict, logger):
    ...
    mod_dict['install_dir'] = gs.base_dir + gs.sl + "build" + gs.sl + mod_dict['system'] + gs.sl + get_label(mod_dict['compiler']) + gs.sl + get_label(mod_dict['mpi']) + gs.sl + mod_dict['code'] + gs.sl + mod_dict['opt_label'] + gs.sl + mod_dict['version'] + gs.sl + "install"
...
def test_mod_file(module, logger):
    ...
    if len(nomatch) > 0:
        ...
        if gs.exit_on_missing:
            exception.error_and_quit(logger, "Missing module parameters were found in module file and exit_on_missing=True in settings.cfg")
...
def make_mod(template_file, general_opts, build_opts, mod_opts, logger):
    ...
    mod_path = gs.base_dir + gs.sl + "build" + gs.sl + "modulefiles" + gs.sl + mod_dict['system'] + gs.sl + get_label(mod_dict['compiler']) + gs.sl + get_label(mod_dict['mpi']) + gs.sl + mod_dict['code'] + gs.sl + mod_dict['opt_label']
