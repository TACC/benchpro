...
if (not os.path.isfile(tool_dir + "/" + check_exe)):
	write_log(f, tool_dir + "/" + check_exe +
			  " executable not found, something is not right...\n")
	sys.exit(1)
elif(os.stat(tool_dir + "/" + check_exe).st_uid != 0):
	write_log(
		f, "Insufficient privileges, run change_permissions.sh as root. Quitting for now...\n")
	sys.exit(1)
else:
